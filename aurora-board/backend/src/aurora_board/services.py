...
# Board Services
def create_board(db: Session, board: schemas.BoardCreate, current_user_id: int):
    def operation(db_session, board_data, user_id):
        db_board = models.Board(name=board_data.name, description=board_data.description, user_id=user_id)
        db_session.add(db_board)
        return db_board
    return _execute_atomic(db, operation, board, current_user_id)

def get_board(db: Session, board_id: int, current_user_id: int):
    db_board = db.query(models.Board).filter(models.Board.id == board_id).first()
    if db_board is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Board not found")
    if db_board.user_id != current_user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions")
    return db_board

def get_boards(db: Session, current_user_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.Board).filter(models.Board.user_id == current_user_id).offset(skip).limit(limit).all()

def update_board(db: Session, board_id: int, board_update: schemas.BoardUpdate, current_user_id: int):
    def operation(db_session, b_id, b_update, user_id):
        db_board = get_board(db_session, b_id, user_id) # Verification of ownership happens here
        update_data = b_update.model_dump(exclude_unset=True)
        for key, value in update_data.items():
            setattr(db_board, key, value)
        db_session.add(db_board)
        return db_board
    return _execute_atomic(db, operation, board_id, board_update, current_user_id)

def delete_board(db: Session, board_id: int, current_user_id: int):
    def operation(db_session, b_id, user_id):
        db_board = get_board(db_session, b_id, user_id) # Verification of ownership happens here
        db_session.delete(db_board)
        return {"detail": "Board deleted successfully"}
    return _execute_atomic(db, operation, board_id, current_user_id)

# List Services
# For Lists and Cards, ownership is derived from the parent Board.
# So, when creating/accessing lists/cards, we must first verify ownership of the Board.

def create_list(db: Session, list_create_data: schemas.ListSchemaCreate, current_user_id: int):
    def operation(db_session, list_data, user_id):
        # Verify board ownership before creating a list in it
        get_board(db_session, list_data.board_id, user_id)
        ...
        return db_list
    return _execute_atomic(db, operation, list_create_data, current_user_id)

def get_list(db: Session, list_id: int, current_user_id: int):
    db_list = db.query(models.List).filter(models.List.id == list_id).first()
    if db_list is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="List not found")
    # Verify ownership via the board
    get_board(db, db_list.board_id, current_user_id)
    return db_list

def get_lists_by_board(db: Session, board_id: int, current_user_id: int, skip: int = 0, limit: int = 100):
    # Verify board ownership first
    get_board(db, board_id, current_user_id)
...
def update_list(db: Session, list_id: int, list_update: schemas.ListSchemaUpdate):
    def operation(db_session, l_id, l_update):
        ...
        db_session.add(db_list)
        return db_list
    def operation(db_session, l_id, l_update, user_id): # Add user_id parameter
        db_list = get_list(db_session, l_id, user_id) # Pass user_id
        update_data = l_update.model_dump(exclude_unset=True)
        ...
        db_session.add(db_list)
        return db_list
    return _execute_atomic(db, operation, list_id, list_update, current_user_id)

def _update_list_positions_on_delete(db: Session, board_id: int, deleted_item_position: int): # No user_id needed, internal helper
...
def delete_list(db: Session, list_id: int, current_user_id: int):
    def operation(db_session, l_id, user_id):
        db_list = get_list(db_session, l_id, user_id) # Verifies ownership
    ...
    return _execute_atomic(db, operation, list_id, current_user_id)


# Card Services
def create_card(db: Session, card_create_data: schemas.CardCreate, current_user_id: int):
    def operation(db_session, card_data, user_id):
        # Verify ownership of the list (and thus the board) before creating a card
        get_list(db_session, card_data.list_id, user_id)
        ...
        return db_card
    return _execute_atomic(db, operation, card_create_data, current_user_id)

def get_card(db: Session, card_id: int, current_user_id: int):
    db_card = db.query(models.Card).filter(models.Card.id == card_id).first()
    if db_card is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Card not found")
    # Verify ownership via the list's board
    get_list(db, db_card.list_id, current_user_id)
    return db_card

def get_cards_by_list(db: Session, list_id: int, current_user_id: int, skip: int = 0, limit: int = 100):
    # Verify ownership of the list first
    get_list(db, list_id, current_user_id)
...
def update_card(db: Session, card_id: int, card_update: schemas.CardUpdate):
    def operation(db_session, c_id, c_update):
        ...
        db_session.add(db_card)
        return db_card
    def operation(db_session, c_id, c_update, user_id): # Add user_id
        db_card = get_card(db_session, c_id, user_id) # Pass user_id
        update_data = c_update.model_dump(exclude_unset=True)
        ...
        if new_list_id is not None and new_list_id != original_list_id:
            # Card is moving to a new list
            get_list(db_session, new_list_id, user_id) # Ensure new list exists and user owns it
        ...
        db_session.add(db_card)
        return db_card
    return _execute_atomic(db, operation, card_id, card_update, current_user_id) # Pass current_user_id

def delete_card(db: Session, card_id: int, current_user_id: int):
    def operation(db_session, c_id, user_id):
        db_card = get_card(db_session, c_id, user_id) # Verifies ownership
    ...
    return _execute_atomic(db, operation, card_id, current_user_id)

def _update_card_positions_on_delete(db: Session, list_id: int, deleted_item_position: int): # No user_id needed, internal helper
